    ...
    var body: some View{
        GeometryReader{geo in
            ...
            if(dataSources.contains(AccAxis.lateral)){
                strokeManager.createGraph(accAxis:AccAxis.lateral).stroke(Color.red,lineWidth: 2).offset(x:50, y:0)
            }
            if(dataSources.contains(AccAxis.vertical)){
                strokeManager.createGraph(accAxis:AccAxis.vertical).stroke(Color.blue,lineWidth: 2).offset(x:50, y:0)
            }
